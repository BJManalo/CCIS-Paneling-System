    ...
    try {
        ...
        groups.forEach(group => {
            ...
            schedules.forEach(sched => {
                allData.push({
                    id: group.id,
                    scheduleId: sched.id, // Needed for fetching comments
                    groupName: group.group_name,
                    adviser: group.adviser, // Needed for permission check
...
// Redirect or View Logic
window.viewGroup = async (groupId, type) => {
    const item = allData.find(d => d.id == groupId && d.type === type);
    if (!item) return;

    // Check if Adviser
    const loginUser = JSON.parse(localStorage.getItem('loginUser'));
    const userName = (loginUser ? (loginUser.full_name || '') : '').toLowerCase();
    const adviserName = (item.adviser || '').toLowerCase();

    // Loose Check
    const isAdviser = adviserName.includes(userName) || (userName && userName.includes(adviserName));

    if (isAdviser) {
        // Fetch Comments for Adviser View
        try {
            const { data: comments, error } = await supabaseClient
                .from('individual_evaluations')
                .select('panelist_name, comments, recommendations')
                .eq('schedule_id', item.scheduleId);

            if (error) throw error;

            if (!comments || comments.length === 0) {
                alert('No panel comments found for this defense.');
                return;
            }

            let msg = `PANEL COMMENTS for ${item.groupName} - ${item.type}\n====================================\n\n`;
            comments.forEach(c => {
                msg += `[ ${c.panelist_name} ]\n`;
                const cText = c.comments ? c.comments.trim() : 'No general comments.';
                const rText = c.recommendations ? c.recommendations.trim() : '';

                msg += `Comments: ${cText}\n`;
                if (rText) msg += `Recommendations: ${rText}\n`;
                msg += '\n------------------------------------\n';
            });
        ...
        // Standard Instructor View (File)
        // For now, just show alert as placeholder or redirect if needed
        alert(`Viewing Submission File for ${item.groupName}...\n(Feature in progress)`);
    }
}
